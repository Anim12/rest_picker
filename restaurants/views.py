# -*- coding: utf-8 -*-
from __future__ import unicode_literals

from django.shortcuts import render, get_object_or_404
from restaurants.models import Restaurant, City
# Create your views here.

def choose_city(request, food_id):
    rest_list =  Restaurant.objects.filter(food=food_id)
    city_list=[]
    for r in rest_list:
        c = City.objects.get(id=r.city.id)
        if c not in city_list:
            rests = Restaurant.objects.filter(food__id=food_id,
                                              city__id=c.id)
            city_list.append({'city':c, 'count':len(rests)})
    return render(request, 'choose_city.html',
                 {'food_id':food_id, 'city_list': city_list})

def choose_restaurant(request,food_id,city_id):
    rests = Restaurant.objects.filter(food__id=food_id,
                                      city__id=city_id)
    return render(request,'choose_restaurant.html',
                 {'rests':rests})

def restaurant(request, rest_id, message=''):
    rest = Restaurant.objects.get(id=rest_id)
    rest_menu = rest.menu.split(';')
    rest_menu = [e for e in rest_menu if e not in ('',' ')]
    return render(request, 'restaurant.html',
                 {'restaurant':rest, 'rest_menu': rest_menu,
                  'message':message})

def vote(request, rest_id):
    rest = get_object_or_404(Restaurant, pk=rest_id)

    votes = rest.votes
    submitted = request.POST['note_utilisateur']
    average = (votes + int(submitted))/2
    rest.votes = average
    rest.save()

    rest_menu = rest.menu.split(';')
    rest_menu = [e for e in rest_menu if e not in ('',' ')]

    message='Thanks for your vote!'
    return render(request, 'restaurant.html',
                 {'restaurant':rest, 'rest_menu': rest_menu,
                  'message':message})
